...
float sin(float angle){
  ...
  int32 INTangle=((uint32)angle)%360;
  int32 sinValue=0;
  if(INTangle<90){
    sinValue = sineLookTable[INTangle*4];
  }
  else if(INTangle<180){
    sinValue = sineLookTable[360-(INTangle-90)*4];
  }
  else if(INTangle<270){
    sinValue = -sineLookTable[(INTangle-180)*4];
  }
  else if(INTangle<360){
    sinValue = -sineLookTable[360-(INTangle-270)*4];
  }
  return (float)sinValue/255.0;
}
